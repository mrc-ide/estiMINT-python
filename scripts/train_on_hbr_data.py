"""
Train estiMINT HBR model using MINTelligence data.

This script:
1. Loads the prepared HBR data from prepare_hbr_data.py
2. Trains XGBoost model (hbr_y9 + interventions -> EIR) using K-fold CV and QMAP calibration
3. Saves model to output/hbr_retrained/
"""
...
# Configuration
DATA_PATH = "/home/cosmo/Documents/Repos/estimint/output/hbr_data/hbr_training_data.csv"
OUTPUT_DIR = "/home/cosmo/Documents/Repos/estimint/output/hbr_retrained"
K_FOLDS = 10
K_STRATA = 16
SEED = 42
...
def main():
    print("=" * 80)
    print("Training estiMINT HBR Model (hbr_y9 + interventions -> EIR)")
    print("=" * 80)
    ...
    if not data_path.exists():
        ...
        print("Please run prepare_hbr_data.py first")
        return 1
    ...
    # Define features — same as prevalence model but with hbr_y9 instead of prev_y9
    features = ["dn0_use", "Q0", "phi_bednets", "seasonal", "itn_use", "irs_use", "hbr_y9"]

    # Transform EIR to log10
    DT["eir_log10"] = np.log10(DT["eir"])

    # XGBoost parameters (same as prevalence model)
    # Monotone constraint: hbr_y9 (index 6) must be positively correlated with EIR
    # More mosquito bites → more infectious bites, all else equal
    # Features: ["dn0_use", "Q0", "phi_bednets", "seasonal", "itn_use", "irs_use", "hbr_y9"]
    #             0         0       0              0           0          0          +1
    xgb_params = {
    ...
    # Reorder cluster IDs by center value
    ...
    oof_metrics.to_csv(dir_metric / f"eir_OOF_metrics_K{K_FOLDS}CV.csv", index=False)
    print("\n" + str(oof_metrics))
    ...
    xgb_final.save_model(str(dir_models / "eir_xgb_HBR_FINAL.model"))
    ...
    test_metrics = pd.DataFrame({
        "set": ["Test"],
        "R2": [r2(obs_eir_test, pred_eir_test)],
        "bias": [np.mean(pred_eir_test - obs_eir_test)],
        "RMSE": [rmse(obs_eir_test, pred_eir_test)],
        "MAE": [mae(obs_eir_test, pred_eir_test)],
    })
    test_metrics.to_csv(dir_metric / "eir_test_metrics.csv", index=False)
    print("\n" + str(test_metrics))

    # Save diagnostic plots
    plot_obs_pred(
        obs_eir_test, pred_eir_test,
        f"EIR — Observed vs Predicted (XGBoost HBR, K={K_FOLDS} CV, QMAP+Scale, test)",
        str(dir_plots / "eir_obs_vs_pred_xgb_HBR_QMAP_SCALE_test.png"),
    ...
    preprocess = {
        ...
        "inverse": "pow10",
        "hbr_filter": {
            "note": "Trained on MINTelligence data, HBR > 0 (Im > 0)"
        },
        "training_data": {
            "source": "MINTelligence HBR_malaria_simulations_4096.duckdb",
            "n_rows": len(DT),
    ...
    # Save pickle bundle
    with open(dir_models / "estiMINT_HBR_model.pkl", "wb") as f:
        pickle.dump(model_bundle, f, protocol=pickle.HIGHEST_PROTOCOL)

    print("\n" + "=" * 80)
    print("Training Complete!")
    print("=" * 80)
    print(f"\nModel saved to: {dir_models}/estiMINT_HBR_model.pkl")

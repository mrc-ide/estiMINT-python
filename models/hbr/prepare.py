...
- eir_to_hbr_training: EIR->HBR model (prev_y9 >= 0.01 AND hbr_y9 > 0)
...
SOURCE = HERE.parents[1] / "datasets" / "estimint_simulations_y9.parquet"
...
def prepare():
    ...
    hbr.to_parquet(HERE / "hbr_training.parquet", index=False)
    ...
    assert len(e2h) == 12874, f"expected 12,874 eir_to_hbr rows, got {len(e2h):,}"
    assert not e2h.isna().any().any(), "unexpected NaN in eir_to_hbr view"
    e2h.to_parquet(HERE / "eir_to_hbr_training.parquet", index=False)

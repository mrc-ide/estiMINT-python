"""Train the HBR->EIR model (hbr_y9 + interventions -> EIR).

Same pipeline as the prevalence model but with hbr_y9 as the monotone feature.
Reads models/hbr/hbr_training.parquet; writes the model, booster, metrics and plot
into this folder.
"""
...
DATA_PATH = HERE / "hbr_training.parquet"
OUTPUT_DIR = HERE
K_FOLDS = 10
K_STRATA = 16
SEED = 42
...
def main():
    ...
    # monotone constraint: hbr_y9 (index 6) positively correlated with EIR
    xgb_params = {
    ...
    preprocess = {
        ...
        "training_data": {
            "source": "datasets/estimint_simulations_y9.parquet (hbr_y9 > 0)",
            "n_rows": len(DT),
    ...
    with open(OUTPUT_DIR / "estiMINT_HBR_model.pkl", "wb") as f:
        pickle.dump(model_bundle, f, protocol=pickle.HIGHEST_PROTOCOL)

    print(f"\nModel saved to: {OUTPUT_DIR}/estiMINT_HBR_model.pkl")
    return 0

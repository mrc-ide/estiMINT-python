"""Train the prevalence->EIR model (prev_y9 + interventions -> EIR).

XGBoost with k-means strata on log10(EIR), 10-fold CV, and QMAP+scale calibration.
Reads models/prevalence/training.parquet; writes the model, booster, metrics and plot
into this folder.
"""
...
DATA_PATH = HERE / "training.parquet"
OUTPUT_DIR = HERE
K_FOLDS = 10
K_STRATA = 16
SEED = 42
...
def main():
    ...
    xgb_final.save_model(str(OUTPUT_DIR / "eir_xgb_FINAL.model"))
    ...
    with open(OUTPUT_DIR / "estiMINT_model.pkl", "wb") as f:
        pickle.dump(model_bundle, f, protocol=pickle.HIGHEST_PROTOCOL)

    print(f"\nModel saved to: {OUTPUT_DIR}/estiMINT_model.pkl")
    return 0
